...
// thx for https://github.com/issuimo/UnityResolve.hpp
...
public:
	...
	template <typename Return, typename... Args>
	static auto InvokeM(void* funcAddress, Args... args) -> Return {
		if (funcAddress != nullptr) {
			// Calculate the actual function address
			auto finalAddress = reinterpret_cast<void*>(
				reinterpret_cast<uintptr_t>(hmodule_) + reinterpret_cast<uintptr_t>(funcAddress)
				);

			// Call the function using the calculated address
			return reinterpret_cast<Return(UNITY_CALLING_CONVENTION*)(Args...)>(finalAddress)(args...);
		}
		throw std::logic_error("Function address is null");
	}
...
public:
	...
	public:
		...
		enum class CameraType { Game = 1, SceneView = 2, Preview = 4, VR = 8, Reflection = 16 };
		...
		struct Camera : Component {
			...
			auto GetCameraType() -> CameraType {
				...
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Camera")
						->Get<Method>("get_cameraType");
				}
				if (method) {
					return method->Invoke<CameraType>(this);
				}
				return CameraType::Game;
			}
			...
			void SetCameraType(CameraType value) {
				...
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Camera")
						->Get<Method>("set_cameraType");
				}
				if (method) {
					method->Invoke<void>(this, value);
				}
			}
		};
		...
		struct Rigidbody : Component {
			...
			auto SetAngularVelocity(Vector3 value) -> void {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("Rigidbody")->Get<Method>(mode_ == Mode::Mono ? "set_angularVelocity_Injected" : "set_angularVelocity");
				if (mode_ == Mode::Mono && method) return method->Invoke<void>(this, &value);
				if (method) return method->Invoke<void>(this, value);
			}

			auto SetInterpolation(UnityResolve::UnityType::RigidbodyInterpolation value) -> void {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("Rigidbody")->Get<Method>("set_interpolation");
				if (method) return method->Invoke<void>(this, static_cast<int>(value)); 
			}
		};
		...
		struct CharacterController : Collider {
			auto SetEnabled(bool value) -> void {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("CharacterController")->Get<Method>("set_enabled");
				if (method) method->Invoke<void>(this, value);
			}

			auto GetEnabled() -> bool {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("CharacterController")->Get<Method>("get_enabled");
				if (method) return method->Invoke<bool>(this);
				return false;
			}

			auto SimpleMove(UnityResolve::UnityType::Vector3 speed) -> bool {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("CharacterController")->Get<Method>("SimpleMove_Injected");
				if (method) {
					return method->Invoke<bool>(this, &speed);
				}
				return false;
			}

			auto Move(UnityResolve::UnityType::Vector3 motion) -> UnityResolve::UnityType::CollisionFlags {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("CharacterController")->Get<Method>("Move_Injected");
				if (method) {
					return method->Invoke<UnityResolve::UnityType::CollisionFlags>(this, &motion);
				}
				return UnityResolve::UnityType::CollisionFlags::None;
			}

			auto GetVelocity() -> UnityResolve::UnityType::Vector3 {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("CharacterController")->Get<Method>("get_velocity_Injected");
				if (method) {
					UnityResolve::UnityType::Vector3 velocity;
					method->Invoke<void>(this, &velocity);
					return velocity;
				}
				return {};
			}
			...
			auto IsGrounded() -> bool {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("CharacterController")->Get<Method>("IsGrounded");
				if (method) return method->Invoke<bool>(this);
				return false;
			}
			...
			auto GetRadius() -> float {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("CharacterController")->Get<Method>("get_radius");
				if (method) return method->Invoke<float>(this);
				return 0.0f;
			}
			...
			auto GetHeight() -> float {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("CharacterController")->Get<Method>("get_height");
				if (method) return method->Invoke<float>(this);
				return 0.0f;
			}
			...
			auto SetHeight(float value) -> void {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("CharacterController")->Get<Method>("set_height");
				if (method) method->Invoke<void>(this, value);
			}

			auto SetRadius(float value) -> void {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("CharacterController")->Get<Method>("set_radius");
				if (method) method->Invoke<void>(this, value);
			}

			auto GetCenter() -> UnityResolve::UnityType::Vector3 {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("CharacterController")->Get<Method>("get_center_Injected");
				if (method) {
					UnityResolve::UnityType::Vector3 center;
					method->Invoke<void>(this, &center);
					return center;
				}
				return {};
			}

			auto SetCenter(UnityResolve::UnityType::Vector3 value) -> void {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("CharacterController")->Get<Method>("set_center_Injected");
				if (method) method->Invoke<void>(this, &value);
			}

			auto GetSkinWidth() -> float {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("CharacterController")->Get<Method>("get_skinWidth");
				if (method) return method->Invoke<float>(this);
				return 0.0f;
			}
		};
		...
		struct VertexAttributeDescriptor {
			...
			VertexAttributeDescriptor(
				...
				VertexAttributeFormat fmt = VertexAttributeFormat::Float32,
				int32_t dim = 3,
				int32_t strm = 0)
				: attribute(attr), format(fmt), dimension(dim), stream(strm) {
			}
			...
			int32_t GetHashCode() const {
				return static_cast<int32_t>(attribute) ^
					(static_cast<int32_t>(format) << 8) ^
					(dimension << 16) ^
					(stream << 24);
			}

			bool Equals(const VertexAttributeDescriptor& other) const {
		...
		struct Mesh : UnityObject {
			static void Internal_Create(Mesh* mesh) {
				static Method* method;
				if (!method) {
					method = Get("UnityEngine.CoreModule.dll")->Get("Mesh")->Get<Method>("MeshScripting::CreateMesh");
				}
				if (method) {
					method->Invoke<void>(nullptr, mesh);
				}
			}
			...
			auto GetIndexFormat() -> IndexFormat {
				static Method* method = nullptr;
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Mesh")
						->Get<Method>("get_indexFormat");
				}
				if (method) {
					return method->Invoke<IndexFormat>(this);
				}
			...
			void SetIndexFormat(IndexFormat format) {
				...
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Mesh")
						->Get<Method>("set_indexFormat");
				}
				if (method) {
					method->Invoke<void>(this, format);
				}
			}

			void SetVertexBufferParams(int vertexCount, VertexAttributeDescriptor* attributes, int attributeCount) {
				static Method* method;
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Mesh")
						->Get<Method>("MeshScripting::SetVertexBufferParamsFromArray");
				}
				if (method) {
					method->Invoke<void>(this, vertexCount, attributes, attributeCount);
				}
			}

			void SetVertexBufferData(int stream, void* data, int dataStart, int meshBufferStart, int count, int elemSize, MeshUpdateFlags flags) {
				static Method* method;
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Mesh")
						->Get<Method>("MeshScripting::InternalSetVertexBufferData");
				}
				if (method) {
					method->Invoke<void>(this, stream, data, dataStart, meshBufferStart, count, elemSize, flags);
				}
			}

			auto GetIndexCount(int submesh) -> uint32_t {
				static Method* method;
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Mesh")
						->Get<Method>("MeshScripting::GetIndexCount");
				}
				if (method) {
					return method->Invoke<uint32_t>(this, submesh);
				}
				return 0;
			}

			auto GetTriangles(int submesh, bool applyBaseVertex) -> std::vector<int> {
				static Method* method;
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Mesh")
						->Get<Method>("MeshScripting::GetTriangles");
				}
				if (method) {
					int* triangles = method->Invoke<int*>(this, submesh, applyBaseVertex);
					int count = GetIndexCount(submesh);
					return std::vector<int>(triangles, triangles + count);
				}
				return {};
			}

			void SetIndices(int submesh, MeshTopology topology, IndexFormat indicesFormat, void* indices, int arrayStart, int arraySize, bool calculateBounds, int baseVertex) {
				static Method* method;
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Mesh")
						->Get<Method>("SetMeshIndicesFromScript");
				}
				if (method) {
					method->Invoke<void>(this, submesh, topology, indicesFormat, indices, arrayStart, arraySize, calculateBounds, baseVertex);
				}
			}

			auto GetVertices() -> std::vector<Vector3> {
				static Method* method;
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Mesh")
						->Get<Method>("get_vertices");
				}
				if (method) {
					Vector3* vertices = method->Invoke<Vector3*>(this);
					int count = GetVertexCount();
					return std::vector<Vector3>(vertices, vertices + count);
				}
				return {};
			}
			...
			void SetVertices(const std::vector<Vector3>& vertices) {
				...
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Mesh")
						->Get<Method>("set_vertices");
				}
				if (method) {
					method->Invoke<void>(this, vertices.data(), static_cast<int>(vertices.size()));
				}
			}
			...
			void Clear(bool keepVertexLayout) {
				...
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Mesh")
						->Get<Method>("ClearImpl");
				}
				if (method) {
					method->Invoke<void>(this, keepVertexLayout);
				}
			}

			void RecalculateBounds(MeshUpdateFlags flags = MeshUpdateFlags::Default) {
				static Method* method;
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Mesh")
						->Get<Method>("RecalculateBoundsImpl");
				}
				if (method) {
					method->Invoke<void>(this, flags);
				}
			}

			auto GetVertexCount() -> int {
				static Method* method;
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Mesh")
						->Get<Method>("GetVertexCount");
				}
				if (method) {
		...
		struct CapsuleCollider : Collider {
			...
			auto SetRadius(float radius) -> void {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("CapsuleCollider")->Get<Method>("set_radius");
				if (method) return method->Invoke<void>(this, radius);
			}
		};
		...
		struct Renderer : Component {
			...
			auto GetMaterial() -> Material* {
				...
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Renderer")
						->Get<Method>("get_material");
				}
				if (method) {
					return method->Invoke<Material*>(this);
				}
				return 0;
			}
		};

		struct Material : UnityObject 
		{
			/*static void CreateWithShader(Material* self, Shader* shader) {
				...
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("MaterialScripting::CreateWithShader");
				}
				if (method) {
					method->Invoke<void>(nullptr, self, shader);
				}
			}*/

			static void CreateWithMaterial(Material* self, Material* source) {
				static Method* method = nullptr;
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("MaterialScripting::CreateWithMaterial");
				}
				if (method) {
					method->Invoke<void>(nullptr, self, source);
				}
			}

			static void CreateWithString(Material* self) {
				static Method* method = nullptr;
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("MaterialScripting::CreateWithString");
				}
				if (method) {
					method->Invoke<void>(nullptr, self);
				}
			}
			...
			/*auto GetShader() -> Shader* {
				...
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("get_shader");
				}
				if (method) {
					return method->Invoke<Shader*>(this);
				}
				return nullptr;
			}
			...
			void SetShader(Shader* shader) {
				...
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("set_shader");
				}
				if (method) {
					method->Invoke<void>(this, shader);
				}
			}*/
			...
			auto GetColor() -> Color {
				...
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("get_color_Injected");
				}
				if (method) {
					Color* color;
					method->Invoke<void>(this, &color);
					return *color;
				}
				return {};
			}
			...
			/*auto GetMainTexture() -> Texture* {
				static Method* method = nullptr;
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("get_mainTexture");
				}
				if (method) {
					return method->Invoke<Texture*>(this);
				}
				return nullptr;
			}*/
			...
			/*void SetMainTexture(Texture* texture) {
				...
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("set_mainTexture");
				}
				if (method) {
					method->Invoke<void>(this, texture);
				}
			}*/

			auto GetMainTextureOffset() -> Vector2 {
				static Method* method = nullptr;
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("get_mainTextureOffset_Injected");
				}
				if (method) {
					Vector2 offset;
					method->Invoke<void>(this, &offset);
					return offset;
				}
				return {};
			}
			...
			void SetMainTextureOffset(const Vector2& offset) {
				...
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("set_mainTextureOffset_Injected");
				}
				if (method) {
					method->Invoke<void>(this, &offset);
				}
			...
			auto GetMainTextureScale() -> Vector2 {
				static Method* method = nullptr;
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("get_mainTextureScale_Injected");
				}
				if (method) {
					Vector2 scale;
					method->Invoke<void>(this, &scale);
					return scale;
				}
				return {};
			}
			...
			void SetMainTextureScale(const Vector2& scale) {
				...
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("set_mainTextureScale_Injected");
				}
				if (method) {
					method->Invoke<void>(this, &scale);
				}
			...
			auto HasProperty(int nameID) -> bool {
				static Method* method = nullptr;
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("HasPropertyFromScript");
				}
				if (method) {
					return method->Invoke<bool>(this, nameID);
				}
				return false;
			}

			//auto HasProperty(const std::string& name) -> bool {
			//	int nameID = Shader::PropertyToID(name); // Предполагается, что метод существует
			...
			void SetFloat(int nameID, float value) {
				...
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("SetFloatFromScript");
				}
				if (method) {
					method->Invoke<void>(this, nameID, value);
				}
			}
			...
			auto GetFloat(int nameID) -> float {
				static Method* method = nullptr;
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("GetFloatFromScript");
				}
				if (method) {
					return method->Invoke<float>(this, nameID);
				}
				return 0.0f;
			}
			...
			void SetColor(int nameID, const Color& value) {
				static Method* method = nullptr;
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("SetColorFromScript_Injected");
				}
				if (method) {
					method->Invoke<void>(this, nameID, &value);
				}
			...
			auto GetColor(int nameID) -> Color {
				static Method* method = nullptr;
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("GetColorFromScript_Injected");
				}
				if (method) {
					Color* color;
					method->Invoke<void>(this, nameID, &color);
					return *color;
				}
				return {};
			}
			...
			/*void SetTexture(int nameID, Texture* texture) {
				...
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("SetTextureFromScript");
				}
				if (method) {
					method->Invoke<void>(this, nameID, texture);
				}
			}*/

			/*void SetTexture(const std::string& name, Texture* texture) {
				int nameID = Shader::PropertyToID(name);
				SetTexture(nameID, texture);
			}*/
			...
			/*auto GetTexture(int nameID) -> Texture* {
				...
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("GetTextureFromScript");
				}
				if (method) {
					return method->Invoke<Texture*>(this, nameID);
				}
				return nullptr;
			}*/
			...
			// Свойство renderQueue
			auto GetRenderQueue() -> int {
				static Method* method = nullptr;
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("GetActualRenderQueue");
				}
				if (method) {
					return method->Invoke<int>(this);
				}
			...
			void SetRenderQueue(int queue) {
				...
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("SetCustomRenderQueue");
				}
				if (method) {
					method->Invoke<void>(this, queue);
				}
			}
			...
			void EnableKeyword(const std::string& keyword) {
				...
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("EnableKeyword");
				}
				if (method) {
					method->Invoke<void>(this, keyword);
				}
			}
			...
			auto IsKeywordEnabled(const std::string& keyword) -> bool {
				static Method* method = nullptr;
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("IsKeywordEnabled");
				}
				if (method) {
					return method->Invoke<bool>(this, keyword);
				}
				return false;
			}

			auto GetShaderKeywords() -> std::vector<std::string> {
				static Method* method = nullptr;
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("MaterialScripting::GetShaderKeywords");
				}
				if (method) {
					std::string* keywords = method->Invoke<std::string*>(this);
					return std::vector<std::string>(keywords, keywords + 1);
				}
				return {};
			}
			...
			void SetShaderKeywords(const std::vector<std::string>& keywords) {
				...
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("MaterialScripting::SetShaderKeywords");
				}
				if (method) {
					method->Invoke<void>(this, keywords.data(), static_cast<int>(keywords.size()));
				}
			}
			...
			void CopyPropertiesFromMaterial(Material* mat) {
				...
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("MaterialScripting::CopyPropertiesFrom");
				}
				if (method) {
					method->Invoke<void>(this, mat);
				}
			}

			auto SetPass(int pass) -> bool {
				static Method* method = nullptr;
				if (!method) {
					method = UnityResolve::Get("UnityEngine.CoreModule.dll")
						->Get("Material")
						->Get<Method>("MaterialScripting::SetPass");
				}
				if (method) {
					return method->Invoke<bool>(this, pass);
				}
				return false;
			}
		};
		...
		static auto AddrM(void* address) {
			auto finalAddress = reinterpret_cast<void*>(
				reinterpret_cast<uintptr_t>(hmodule_) + reinterpret_cast<uintptr_t>(address)
				);
			return finalAddress;
		}
	};
...
#define FOR_EACH_COMPONENT(gPtr, componentType, dllName, componentName, componentVar) \
    for (const auto componentVar : (gPtr)->GetGameObject()->GetComponentsInChildren<componentType*>(UnityResolve::Get(dllName)->Get(componentName)))

#define GET_CLASS(dllName, className, classVar) \
    if (const auto classVar = UnityResolve::Get(dllName)->Get(className))
